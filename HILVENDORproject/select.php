...
<?php 
  if(isset($_GET['id'])){
    ...
    $row = mysqli_fetch_array($result,MYSQLI_ASSOC);
     ...
     $sqll = sprintf("INSERT INTO vendordetails (namev,firm,cont, cost, code, gst, transcost, cdate, itemr, quantityr, specr, dater, locr) VALUES ('%s' ,'%s','$cont','$cat' , '$code','$gst','$tranc','$cd','%s', '%s' , '%s' , '%s' , '%s')", mysqli_real_escape_string($db,$_POST['namev']),mysqli_real_escape_string($db,$_POST['firm']),mysqli_real_escape_string($db,$row['item']),mysqli_real_escape_string($db,$row['quantity']),mysqli_real_escape_string($db,$row['specification']), mysqli_real_escape_string($db,$row['date']),mysqli_real_escape_string($db,$row['location']));
...
?>
    ...
    <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="number" class="form-control" name="cont"  aria-describedby="emailHelp" required>
        ...
        <div class="form-group  ">
            ...
            <input type="number" class="form-control" name="gst"  step="0.001" id="exampleInputCode" required>
          </div>
          <div class="form-group  ">
